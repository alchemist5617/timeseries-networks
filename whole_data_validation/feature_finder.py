...
def min_MSE_finder_index_V(count, result_sst, link, df_sst, V, index = 60 , tau=-1, n_estimators=100, max_depth=5):
    result = []
    link = link[link[:,1] <= tau]
...
def best_link_finder_V(count, data_sst, link, df_sst, V, index = 60 , tau=-1, n_estimators=100, max_depth=5):
    
    result, link = min_MSE_finder_index_V(count, data_sst, link, df_sst, V, index, tau)
